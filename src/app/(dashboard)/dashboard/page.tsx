    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <section className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-card p-6">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Researchers by Domain</h2>
          <ResearchAreaDonutChart data={stats?.area_breakdown ?? []} />
        </div>
        <div className="rounded-xl border border-border bg-card p-6">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Model Status</h2>
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between"><dt className="text-muted-foreground">Algorithm</dt><dd className="font-mono text-foreground">{stats?.model_version ?? 'XGBoost v1'}</dd></div>
            <div className="flex justify-between"><dt className="text-muted-foreground">Last Trained</dt><dd className="font-mono text-foreground">{stats?.model_trained_at ? new Date(stats.model_trained_at).toLocaleDateString() : '—'}</dd></div>
            <div className="flex justify-between"><dt className="text-muted-foreground">Training Samples</dt><dd className="font-mono text-foreground">{stats?.n_training_samples ?? 800}</dd></div>
            <div className="flex justify-between"><dt className="text-muted-foreground">Macro F1</dt><dd className="font-mono text-[#00d68f]">{stats?.macro_f1 ? stats.macro_f1.toFixed(3) : '—'}</dd></div>
          </dl>
        </div>
      </section>
