      ...
      <div className="grid grid-cols-2 gap-1">
        {Object.entries(checks).map(([label, ok]) => (
          <div key={label} className="flex items-center gap-1.5">
            {ok ? <Check className="h-3 w-3 text-[#00d68f] shrink-0" /> : <X className="h-3 w-3 text-zinc-700 shrink-0" />}
...
export default function RegisterPage() {
  ...
  const inputCls = 'bg-[rgba(255,255,255,0.04)] border-[rgba(255,255,255,0.08)] text-zinc-100 placeholder:text-zinc-700 focus:border-[rgba(0,214,143,0.4)] focus:ring-[rgba(0,214,143,0.15)] transition-colors'
    ...
    <div className="w-full max-w-sm animate-fade-up" style={{ animationDuration: '0.45s' }}>
      ...
      >
        <div className="mb-6">
          ...
          <p className="text-sm text-zinc-500 mt-1">Free forever · 20 searches/day · no credit card</p>
        </div>

        <form
          onSubmit={handleSubmit((d) => registerUser({ email: d.email, password: d.password, full_name: d.full_name }))}
          className="space-y-4"
        >
        ...
        <p className="text-center text-xs text-zinc-600 mt-6">
          ...
          <Link href="/login" className="text-[#00d68f] hover:underline font-medium">Sign in</Link>
        </p>
      </div>
    </div>
  )
