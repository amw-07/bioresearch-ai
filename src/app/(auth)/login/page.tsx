      ...
      >
        ...
        <form onSubmit={handleSubmit((d) => login(d))} className="space-y-5">
          <div className="space-y-1.5">
            <Label htmlFor="email" className="text-xs font-medium uppercase tracking-widest text-zinc-500">Email</Label>
            <Input
              id="email"
              type="email"
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <Label htmlFor="password" className="text-xs font-medium uppercase tracking-widest text-zinc-500">Password</Label>
              <Link href="/reset-password" className="text-xs text-[#00d68f] hover:underline">Forgot?</Link>
            </div>
            <Input
              id="password"
              type="password"
          ...
          >
            {isLoggingIn ? <><Loader2 className="h-4 w-4 animate-spin" /> Signing in…</> : 'Sign in'}
          </button>
        </form>

        <p className="text-center text-xs text-zinc-600 mt-6">
          Don&apos;t have an account?{' '}
          <Link href="/register" className="text-[#00d68f] hover:underline font-medium">Create one free</Link>
        </p>
      </div>
    </div>
  )
