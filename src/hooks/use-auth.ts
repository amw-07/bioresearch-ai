...
export function useAuth() {
  ...
  const registerMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({ title: 'Success', description: data.message || 'Account created. Please log in.' })
      router.push('/login')
    },
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.response?.data?.detail || error.response?.data?.message || 'Registration failed',
        variant: 'destructive',
      })
    },
  })
